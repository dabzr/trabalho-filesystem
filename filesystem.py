...
def get_dir(self, current_dir, path: str):
    if path == "/":
        return self.root
    if path.startswith("/"):
        dir = self.root
...
def change_directory(self, current_dir, path):
    if not path:
        return self.root
    return self.get_dir(current_dir, path[0]) or current_dir


def make_directory(self, current_dir, path):
    if not path:
        print("mkdir: missing operand")
        return
    if "/" not in path[0]:
        entries = current_dir.get_entries()
        if path[0] in entries:
            print(f"mkdir: '{path[0]}' already exists")
            return
        new_dir = Directory(path[0], parent=current_dir, fs=self)
        entries[path[0]] = new_dir.inode_idx
        current_dir.update_entries(entries)
        return

    p = path[0].rpartition("/")
    dir = self.get_dir(current_dir, p[0])
    if dir is None:
        return
    entries = dir.get_entries()
    if p[-1] in entries:
        ...
        return
    new_dir = Directory(p[-1], parent=dir, fs=self)
    entries[p[-1]] = new_dir.inode_idx
    dir.update_entries(entries)


def remove_directory(self, current_dir, path):
    dir = self.get_dir(current_dir, path[0])
...
def make_file(self, current_dir, path):
    if len(path) < 2:
...
def remove_file(self, current_dir, path):
    if not path:
...
def list_directory(self, current_dir, path=None):
    if not path:
        dir = current_dir
    else:
        dir = self.get_dir(current_dir, path[0])
